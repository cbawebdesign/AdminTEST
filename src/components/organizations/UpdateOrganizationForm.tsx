...
const UpdateOrganizationForm = () => {
  ...
  const onSubmit = useCallback(
    async (organizationName: string,organizationlastName: string, logoFile: Maybe<File>) => {
  ...
  return (
    <form
      onSubmit={handleSubmit((value) => {
        return onSubmit(value.name, value.lastName, getLogoFile(value.logoURL));
      })}
      className={'space-y-4'}
    >
      <div className={'flex flex-col space-y-4'}>
        ...
        <TextField>
          <TextField.Label>
            <Trans i18nKey={'organization:organizationlastNameInputLabel'} />
